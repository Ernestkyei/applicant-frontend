...
import { GraduationCap, Menu, X, Key, LogOut } from "lucide-react";
...
export function Header({ 
  ...
  // Check if user is logged in (using access code)
  const hasAccess = isAuthenticated || !!localStorage.getItem('accessToken');
  ...
  return (
    <nav className="flex justify-between items-center px-6 md:px-12 py-5 bg-gray-900 w-full">
      ...
      <Link to="/" className="flex items-center gap-2.5 hover:opacity-80 transition-opacity">
        ...
        <span className="text-[17px] font-semibold text-white">Admissions Registry</span>
      </Link>
      ...
      {mobileMenuOpen && (
        <div className="md:hidden absolute top-[72px] left-0 right-0 bg-gray-800 px-6 py-4 flex flex-col gap-4 w-full z-50">
          ...
          {hasAccess ? (
            ...
            // Not logged in mobile view - show access code button
            <Link
              to="/access-code"
